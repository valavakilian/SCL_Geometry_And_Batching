...
def main(logs_path, dataset, legend = False, showTPT = False, showTPTLegend = False):
    ...
    return G_Mu_nonCentered_OF_comparison_train, angles_train, NC1_train, last_zero_error_epoch
...
if __name__ == '__main__':
    ...
    def plot_subplot(axs, G_Mu_nonCentered_OF_comparison_train, last_zero_error_epoch, xtick = False, ytick = False, show_arrow = False, legend = False):
        ...
        if show_arrow:
            axs.arrow(last_zero_error_epoch,0.75,50,0.0,width=0.04, head_width=.1, head_length = 10, color = "black")
            axs.text(last_zero_error_epoch + 4, 0.8 + 0.02, "Zero Error", fontsize = 20, weight='bold')
        axs.set_ylim((-0.0, 1.01))
        axs.set_xlim((-0, 351))

        if legend:
            axs.legend(loc='upper right', borderaxespad=0, fontsize=25)
    ...
    G_Mu_nonCentered_OF_comparison_train, angles_train, NC1_train, last_zero_error_epoch = main("../logs/" + args.model + "_CIFAR10_step_SCL_ReLU_noAugment", "CIFAR10", legend = False, showTPT = True, showTPTLegend = False)
    plot_subplot(axs[0,0], angles_train, last_zero_error_epoch, ytick = True, show_arrow = True)

    G_Mu_nonCentered_OF_comparison_train, angles_train, NC1_train, last_zero_error_epoch = main("../logs/" + args.model + "_MNIST_step_SCL_ReLU_noAugment", "MNIST", legend = False, showTPT = True, showTPTLegend = False)
    plot_subplot(axs[0,1], angles_train, last_zero_error_epoch)

    G_Mu_nonCentered_OF_comparison_train, angles_train, NC1_train, last_zero_error_epoch = main("../logs/" + args.model + "_FMNIST_step_SCL_ReLU_noAugment", "FMNIST", legend = False, showTPT = True, showTPTLegend = False)
    plot_subplot(axs[0,2], angles_train, last_zero_error_epoch, legend = True)

    G_Mu_nonCentered_OF_comparison_train, angles_train, NC1_train, last_zero_error_epoch = main("../logs/" + args.model + "_CIFAR10_exp_SCL_ReLU_noAugment", "CIFAR10", legend = False, showTPT = True, showTPTLegend = False)
    plot_subplot(axs[1,0], angles_train, last_zero_error_epoch, ytick = True, xtick = True)

    G_Mu_nonCentered_OF_comparison_train, angles_train, NC1_train, last_zero_error_epoch = main("../logs/" + args.model + "_MNIST_exp_SCL_ReLU_noAugment", "MNIST", legend = False, showTPT = True, showTPTLegend = False)
    plot_subplot(axs[1,1], angles_train, last_zero_error_epoch, xtick = True)

    G_Mu_nonCentered_OF_comparison_train, angles_train, NC1_train, last_zero_error_epoch = main("../logs/" + args.model + "_FMNIST_exp_SCL_ReLU_noAugment", "FMNIST", legend = False, showTPT = True, showTPTLegend = False)
    plot_subplot(axs[1,2], angles_train, last_zero_error_epoch, xtick = True)

    plt.subplots_adjust(wspace=0.02, hspace=0.02)

    plt.savefig("Angles_Convergence_plots.pdf",bbox_inches='tight', dpi=1200)
    plt.show()
