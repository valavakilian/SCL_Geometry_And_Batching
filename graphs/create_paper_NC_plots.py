...
def main(logs_path, dataset, legend = False, showTPT = False, showTPTLegend = False):
    ...
    for R in [1,5,10,50,100]:
        ...
        # total_accuracy_train = calculate_accuracies(graph.train_acc_perclass, classes, maj_classes, min_classes)
        # total_accuracy_test = calculate_accuracies(graph.test_acc_perclass, classes, maj_classes, min_classes)
...
if __name__ == '__main__':
    ...
    def plot_subplot(axs, G_Mu_nonCentered_OF_comparison_train, last_zero_error_epoch, xtick = False, ytick = False, show_arrow = False, legend = False):
        ...
        if show_arrow:
            axs.arrow(last_zero_error_epoch,0.9,50,0.0,width=0.4, head_width=1.0, head_length = 10, color = "black")
            axs.text(last_zero_error_epoch + 4, 0.9 + 0.7, "Zero Error", fontsize = 20, weight='bold')
        axs.set_ylim((2 * 10**(-4), 2 * 10**(2)))
        axs.set_xlim((-0, 351))
    ...
    plot_subplot(axs[0,0], NC1_train, last_zero_error_epoch, ytick = True, show_arrow = True)
    ...
    plot_subplot(axs[0,1], NC1_train, last_zero_error_epoch)
    ...
    plot_subplot(axs[0,2], NC1_train, last_zero_error_epoch, legend = True)
    ...
    plot_subplot(axs[1,0], NC1_train, last_zero_error_epoch, ytick = True, xtick = True)
    ...
    plot_subplot(axs[1,1], NC1_train, last_zero_error_epoch, xtick = True)
    ...
    plot_subplot(axs[1,2], NC1_train, last_zero_error_epoch, xtick = True)

    plt.subplots_adjust(wspace=0.02, hspace=0.02)

    plt.savefig("NC_Convergence_plots.pdf",bbox_inches='tight', dpi=1200)
    plt.show()
